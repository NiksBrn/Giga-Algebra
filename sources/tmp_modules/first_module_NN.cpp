...
Compare NaturalNumber::COM_NN_D(const NaturalNumber& other) const
{
    ...
    Node* next_node2 = other.Big_int.get_first();//последняя цифра второго числа
    
    int count=0; //если разряд первого числа превысил разряд второго, получит значение 1, иначе 0

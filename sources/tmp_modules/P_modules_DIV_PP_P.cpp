...
Polynomial Polynomial::DIV_PP_P(const Polynomial& other) {
  ...
  NaturalNumber maxDegree = tmp->degree;
  ...
  while (poly != nullptr && poly->degree.COM_NN_D(maxDegree) != Compare::LESS) {
    ...
    temp_poly = temp_poly.SUB_PP_P(
        const_cast<Polynomial&>(other).MUL_PQ_P(coefficient).MUL_Pxk_P(degree));
...
!(GCF.polynomial->get_tail()->coefficient.Trans_Q_Z().POZ_Z_D()==ZERO)) { NMR =
